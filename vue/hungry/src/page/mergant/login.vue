<template>
  <div class="login-wrap">
    <el-form label-position="left" :model="loginForm" :rules="rules" ref="loginForm" label-width="0px" class="demo-ruleForm login-container">
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item prop="code">
            <el-input type="text" v-model="loginForm.code" auto-complete="off" placeholder="图形验证码" @keyup.enter.native="submitForm('loginForm')"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12" class="code-box">
          <div @click="makeCode(4)">
            <s-identify :identifyCode="codeimg"/>
          </div>
        </el-col>
      </el-row>
...
export default {
  ...
  mounted() {
    this.makeCode(4);
    if (getCookie('mergantUser') != '' && getCookie('mergantPwd') != '') {
      this.loginForm.username = getCookie('mergantUser')
      this.loginForm.password = getCookie('mergantPwd')
  ...
  methods: {
    makeCode(l) {
      ...
      for (let i = 0; i < l; i++) {
        this.codeimg += random[Math.floor(Math.random() * random.length)];
      }
      console.log(this.codeimg);
    },
    resultDeal(mergantInfo) {
      if (this.rememberpwd) {
        //保存帐号到cookie，有效期7天
        setCookie('mergantUser', this.loginForm.username, 7);
        //保存密码到cookie，有效期7天
        setCookie('mergantPwd', this.loginForm.password, 7);
      } else {
        delCookie('mergantUser');
        delCookie('mergantPwd');
      }
      console.log(mergantInfo);
      localStorage.setItem('MergantInfo', JSON.stringify(mergantInfo));
    },
    submitForm(formName) {
